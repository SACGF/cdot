...
**Table 1. Fixes applied across the production corpus (N = 32,752).** Each row is one
`clean_hgvs()` fix category, with the count of queries it rescued (turned from unparseable
to parseable) and its share of all rescues. *(Tier 2; frozen constants — values to be
populated from `cdot_private/output/`.)*

| Fix category | Strings rescued | % of rescues |
|---|---|---|
| Whitespace / non-printable removal | _TBD_ | _TBD_ |
| Base re-casing (e.g. `delg` → `delG`) | _TBD_ | _TBD_ |
| Protein-suffix stripping | _TBD_ | _TBD_ |
| Gene/transcript-wrapper repair | _TBD_ | _TBD_ |
| Structural-punctuation repair | _TBD_ | _TBD_ |
| Prefix / kind restoration | _TBD_ | _TBD_ |
| Other | _TBD_ | _TBD_ |
| **Total** | **≈1,600** | **100%** |
