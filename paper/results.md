...
the fraction parseable by biocommons/hgvs from **91.5%** as-submitted to **96.4%** after
cleaning: a **+4.9%** absolute gain (≈1,600 additional strings rescued) with **0
...
|---|---|---|---|
| Whitespace / non-printable removal | `NM_000059.4: c.1A>G` → `NM_000059.4:c.1A>G` | 940 | 58.0% |
| Base re-casing | `NM_000059.4:c.1delg` → `…delG` | 553 | 34.1% |
| Structural-punctuation repair | `NM_000059..4:c.1A>G` → `NM_000059.4:c.1A>G` | 310 | 19.1% |
| Gene/transcript-wrapper repair | `BRCA2(NM_000059.4):c.1A>G` → `NM_000059.4(BRCA2):c.1A>G` | 209 | 12.9% |
| Protein-suffix stripping | `NM_000059.4:c.1A>G p.(Met1?)` → `NM_000059.4:c.1A>G` | 130 | 8.0% |
...
error class under a fixed decision-tree taxonomy. Seven of the eight classes are
repair-relevant and are shown below with synthesised examples; the eighth was
non-HGVS input (81 queries, 6.9% — pasted URLs, report templates, or prose) and is
excluded from the table as there is nothing in it for cleaning to repair.

**Residual error classes after cleaning** (counts and % of the 1,175 residual
queries; examples synthesised from public BRCA2 `NM_000059.4`). *(Tier 2; frozen
constants from `cdot_private/output/`.)*

| Class | Queries | What it is — *example* |
|---|---|---|
| Truncated | 284 (24.2%) | cut off before a complete variant — `NM_000059.4:c.68_69` (range, no edit) |
| No reference | 277 (23.6%) | a bare variant body, no transcript/gene/accession — `c.68_69delAG` |
| Bad accession | 167 (14.2%) | missing prefix, or misplaced/truncated version — `NM000059:c.68del` |
| Edit syntax | 143 (12.2%) | malformed or non-standard edit operation — `NM_000059.4:c.68_69del2` (count shorthand) |
| Trailing / concatenated | 85 (7.2%) | extra characters after a complete variant, or several run together — `NM_000059.4:c.68delAG;c.70A>G` |
| Grammar gap | 81 (6.9%) | legitimate HGVS the biocommons grammar rejects — `NM_000059.4:c.(67_70)del` (uncertain range) |
| Genomic ref in parens | 57 (4.9%) | a genomic `NC_` accession in the gene-symbol slot — `NM_000059.4(NC_000013.11):c.68del` |

The residual falls into four regimes. Roughly half (~48%: Truncated + No reference) is
incomplete or reference-less user input: information the user never supplied, which no
string-level repair can invent. About 38% (Bad accession + Edit syntax + Trailing /
concatenated + Genomic ref in parens) is in principle fixable and marks the frontier for
future cleaning rules. The remaining ~7% is a grammar gap — valid HGVS the parser rejects
rather than the input (Grammar gap class) — and a further ~7% (excluded above) was
non-HGVS junk that should not be parsed at all. Most of what remains is therefore either
out of scope for any repair or a downstream grammar limitation rather than a cdot
deficiency.
