...
- #112 - HGVS cleaning: new ops driven by the search-log corpus - colon used in place of the kind dot (`:c:123` → `:c.123`), parens wrapping the whole accession (`(NM_000059.4):c.…` → `NM_000059.4:c.…`), and a gene-first wrapper missing the colon before the kind (`BRCA1(NM_…)c.…` → `NM_…(BRCA1):c.…`)
- #112 - Cleaning analysis: new `analysis/inject_and_clean.py`, a Tier-1 reproducible cleaning benchmark (no client code change). It injects each `clean_hgvs()` fix category into a seeded sample of clean public ClinVar c.HGVS (`analysis/clean_injection_sample.txt`), then measures exact recovery and regressions; emits `output/facts/cleaning.{json,csv}` for the paper. Per-category injection weights are frozen constants citing the aggregate production rescue-op distribution (no corpus string is read or copied). Run on the committed test data: 100% recovery across all 18 categories over 3,419 injected cases, 0 regressions. Wired into `paper/Snakefile` (`cleaning` rule) and the cleaning Methods/Results sections of the paper
- #112, #28 - Resolution benchmark: `analysis/benchmark_resolution.py --recovery` now routes every string through the single combined `fix_hgvs()` entry point (clean → gene-resolve → version-bump) and attributes each end-to-end rescue to a failure mode by reading the returned `HGVSFix` codes (no client code change). Two passes: an as-is pass (production-mode rescue/false-rescue counts + a regression guard) and a degraded pass (degrade each clean pair per mode → `fix_hgvs()` → resolve vs the ground-truth g.HGVS, with a false-rescue rate). Replaces the previous ad-hoc parse-only cleaning/version-bump experiments
- Benchmark scaling + provider comparison (analysis/paper tooling, no client code change): `analysis/build_clinvar_pairs.py` builds a large (g.HGVS, c.HGVS) set by joining ClinVar's `variant_summary.txt.gz` (transcript c.HGVS) with the ClinVar VCF (normalised genomic CLNHGVS) on ALLELEID; `analysis/compare_providers.py` prints a side-by-side resolution/throughput table from the `--out` JSONs. `benchmark_resolution.py` can now point `--uta` at a locally loaded UTA (`UTA_DB_URL`) and labels runs `uta_local`/`uta_remote`. Measured (GRCh38, identical biocommons engine + shared local SeqRepo): cdot 100% / UTA-local 100% / UTA-remote 99% on RefSeq; on Ensembl cdot 100% vs **UTA 0%** (no Ensembl alignments); cdot ~540 HGVS/s local JSON / ~560 with REST prefetch vs ~17–24 (local UTA) / ~0.1 (remote UTA)
- Paper facts wired up + builds: `output/facts/{coverage,clinvar,benchmark,literature,cleaning}.csv` now populate the Methods/Results/Figures templates (coverage 1,621,073 alignments = 11.5× UTA, 574,584 unique Ensembl, 186,644 unique T2T; ClinVar resolution cdot 99.0% vs UTA 70.3% on a combined RefSeq+Ensembl sample). Migrated the removed vibepaper `pct` filter to `dp(N)%`. Added a `[paper]` optional-dependency group (`snakemake`, `vibepaper`)

### Changed

- #84 - Docs: recommend `FastaSeqFetcher` (chained after SeqRepo) as the default setup, since cdot provides many transcripts SeqRepo lacks - added a tip in the README and strengthened the `docs/fasta_seqfetcher.md` background (docs only, no client code change). The detection/warn-on-mismatch half of #84 is left as separate feature work
